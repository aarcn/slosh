...
def poker_sidebet(master_count_map):
    ...
    Prints the calculated probabilities and RTP.
    ...
    total_triples = math.comb(total_cards, 3)
    straight_sequences = [ranks[i:i + 3] for i in range(len(ranks) - 2)] + [["A", "2", "3"]]

    suited_three_kind_probability = 0
    three_kind_probability = 0
    ...
    for rank in ranks:
        for suit in suits:
            ...
            if suit_count >= 3:
                suited_three_kind_probability += math.comb(suit_count, 3) / total_triples
    ...
    for rank in ranks:
        ...
        if count >= 3:
            three_kind_probability += math.comb(count, 3) / total_triples
    three_kind_probability -= suited_three_kind_probability
    ...
    for suit in suits:
        for sequence in straight_sequences:
            ...
            if count1 > 0 and count2 > 0 and count3 > 0:  # Ensure all three cards exist
    ...
    for suit in suits:
        ...
        if suit_count >= 3:
            ...
            flush_probability += flush_combination / total_triples
    flush_probability -= straight_flush_probability + suited_three_kind_probability
    ...
    no_hand_probability = 1 - (flush_probability + straight_probability +
                               three_kind_probability + straight_flush_probability +
                               suited_three_kind_probability)

    calculated_rtp = 100 + (((flush_probability * 4) + (straight_probability * 10) +
                             (three_kind_probability * 20) + (straight_flush_probability * 30) +
                             (suited_three_kind_probability * 100) + (no_hand_probability * -1)) * 100)
